...
INSERT INTO looking_at_the_moon ( id, fk_brain_id, fk_owner_id ) VALUES ( DEFAULT, ( SELECT id FROM brains WHERE id = 0 ), ( SELECT id FROM owners WHERE id = 0 ) );
...
INSERT INTO impulses ( id, fk_looking_at_the_moon_id, fk_state_name ) VALUES ( ( SELECT id FROM looking_at_the_moon WHERE id = 0 ), ( SELECT name FROM states WHERE name LIKE 'совершенно усыплен' ) );

-- properties for impulses table
INSERT INTO properties ( fk_impulse_id, name )
  VALUES
    ( ( SELECT id FROM impulses WHERE id = 0 ), 'привычный'   ),
    ( ( SELECT id FROM impulses WHERE id = 0 ), 'интуитивный' );
...
INSERT INTO dreams ( id, fk_monolith_id ) VALUES ( DEFAULT, ( SELECT id FROM monoliths WHERE id = 0 ) );

-- images of dreams table
INSERT INTO images ( id, is_real, fk_dream_id )
  VALUES
    ( DEFAULT, true, ( SELECT id FROM dreams WHERE id = 0  ) ),
    ( DEFAULT, true, ( SELECT id FROM dreams WHERE id = 0 ) );
...
INSERT INTO aliens ( id, fk_realty_id )
  VALUES
    ( DEFAULT, ( SELECT id FROM realties WHERE id = 0 ) ),
    ( DEFAULT, ( SELECT id FROM realties WHERE id = 0 ) );
...
INSERT INTO expulsions ( fk_impulse_id, fk_realty_id, is_success )
  VALUES (
    ( SELECT id FROM impulses WHERE id = 0 ),
    ( SELECT id FROM realties WHERE id = 0 ),
    CASE SELECT fk_state_name FROM impulses WHERE id = ( SELECT id FROM impulses WHERE id = 0 )
      WHEN LIKE '%пробужден'
      THEN true 
      ELSE false 
    END
